...
class Permissions extends dbobject
{
    ...
    public function permission_list($data)
    {
        ...
        $columner = array(
            ...
            array('db' => 'id', 'dt' => 9, 'formatter' => function ($d, $row) {
                return "<a title='Edit Permissions' href=\"javascript:void(0)\" onclick=\"myLoadModal('setup/permission_setup.php?op=edit&id=$d','modal_div')\"><i  class='btn btn-primary fas fa-edit'></i></a>
    ...
    public function deletePermission($data)
    {
        $id = $data['id'];
        $delete['is_deleted'] = 1;
        $stmt = $this->doUpdate('permissions', $delete, array(''), array('id' => $id));
        if ($stmt > 0) :
    ...
    public function savePermissions($data)
    {
        ...
        if ($data['operation'] == "new") {
            ...
            if (!$validation['error']) {
                ...
                $count = $this->doInsert('permissions', $data, array('op', 'operation', 'id', 'nrfa-csrf-token-label'));
                if ($count > 0) {
            ...
            $count = $this->doUpdate('permissions', $data, array('op', 'operation', 'id', 'nrfa-csrf-token-label', '_cors_'), array('id' => $data['id']));
            if ($count > 0) {
